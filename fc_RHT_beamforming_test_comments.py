...
#from obspy.signal.invsim import detrend
...
import obspy.core.util.geodetics
import csv
...
def prep_beam(files,matfile,nhours=1,fmax=10.,threshold_std=0.5,onebit=False,
              tempfilter=False,specwhite=True,fact=100,new=True,fftpower=7,freq_int=(0.02,0.4),laura=False):
    if new:
        ...
        for i,_f in enumerate(files):
            ...
            #------------------------------
            # Import the station lat/lon file
            sta_lats, sta_lons, sta_name = [], [], []
            stalatlon_file='/Users/home/civilfra/Desktop/beamforming_testcase/RHT_stalocs.txt'
            with open(stalatlon_file) as f:
                # Create a csv reader object.
                reader = csv.reader(f,delimiter=' ')
                
                # Store the latitudes and longitudes in the appropriate lists.
                for row in reader:
                    sta_lats.append(float(row[2]))
                    sta_lons.append(float(row[1]))
                    sta_name.append(row[0])
            
            # From the name, pull the station lat lons
            # Find station index
            sta_index = sta_name.index(staname)
#            slat = tr.stats.sac.stla
#            slon = tr.stats.sac.stlo
            slat = sta_lats[sta_index]
            slon = sta_lons[sta_index]
            
            #------------------------------
            slons = append(slons,slon)
            slats = append(slats,slat)
            if staname not in stations:
                stations.append(staname)
            tr.data -= tr.data.mean()
            tr.filter("bandpass",freqmin=0.02,freqmax=0.4,corners=2,zerophase=True)
...
#            tr.filter("bandpass",freqmin=0.02,freqmax=0.4,corners=4,zerophase=True)
            ...
            seis0 -= seis0.mean()
	    # Cut seismograms into chunks of length 'step'
            for j in xrange(ntimes):
                ilow = j*step
                iup = (j+1)*step
                seisband[i,j,:] = seis0[ilow:iup]
                seisband[i,j,:] -= seisband[i,j,:].mean()
		# Optional 1-bit normalisation
                if onebit:
                    seisband[i,j,:] = sign(seisband[i,j,:])
	    # Optionally cut off timeseries at constant*std; another way of getting rid of earthquakes 
            tempfilter = False
            if tempfilter:
	...
	# Apply a taper to the start and end of seismogram chunks to
        # avoid ringing in the frequency domain
        ismall = 2**fftpower
        ipick = arange(ismall)
        ...
        for ii in xrange(nfiles):
		    ...
		    # Cut seismogram chunks into again smaller chunks with a power-of-two length
		    # to make the fft more efficient and apply the taper
        ...
        if np.isnan(fseis).any():
            print "NaN found"
            return
	# Only consider frequencies of interest
        ...
        #---------------------------
        # Setup latitude/longitude bounding box for the study area
        LonLref= 33.0
        LonUref= 55.0
        LatLref= 15.0
        LatUref= 32.0
...
#        dist, az, baz = obspy.signal.rotate.gps2DistAzimuth(meanlat,meanlon,lat,lon)
        dist, az, baz = obspy.core.util.geodetics.calcVincentyInverse(meanlat,meanlon,lat,lon)
...
def main(datdir,datafiles,outputdir,nprep=False,nbeam=False,doplot=True,save=False,nostat=20):
    #files = glob.glob(os.path.join(datdir,'ft_*.*HZ.SAC'))
...
#    files = glob.glob(os.path.join(datdir,'[!^ft]*.*HZ.SAC'))
    ...
    if datdir.endswith('/'):
        datdir = datdir[0:-1]
    temp = os.path.basename(datdir)
    matfile1 = outputdir + "%s_%s.mat"%('prep_beam',temp)
#    trZ = SacIO(files[0],headonly=True)
    # SAMPLING RATE USED FOR DATA
    fcdelta = 0.01
#    sample_f = int(round(1./trZ.delta))
    sample_f = int(round(1./fcdelta))
    newprep = not os.path.isfile(matfile1)
    if nprep:
        newprep = True
    fseis,freqs, meanlat, meanlon, slats, slons, dt, seissmall = prep_beam(files,matfile1,onebit=False,
                                                                           ...
                                                                           specwhite=False)
    zetax,theta,slowness,sta_origin_x,sta_origin_y = calc_steer(slats,slons)
    matfile2 = outputdir + "%s_%s.mat"%('beam',temp)
    newbeam = not os.path.isfile(matfile2)
    if nbeam:
        newbeam = True
...
#-----------------------------------
textfilename = '/Users/home/civilfra/Desktop/beamforming_testcase/RHT_001.txt'
datafiles = []
with open(textfilename) as f:
    ...
    # Store the latitudes and longitudes in the appropriate lists.
    for row in reader:
        datafiles.append(row[0])
        
# Original Yannik data directory
#datdir = '/Volumes/GeoPhysics_05/users-data/yannik78/taranaki/sacfiles/5Hz/2002/Jul/2002_7_12_0_0_0'

datdir = '/Volumes/GeoPhysics_14/users-data/civilfra/USGS_SA/SA_RHT_2014_001/' # This directory location is no longer necessary
outputdir = '/Users/home/civilfra/Desktop/beamforming_testcase/output/'
main(datdir,datafiles,outputdir,nbeam=True,nprep=True,doplot=True,save=True,nostat=13)
